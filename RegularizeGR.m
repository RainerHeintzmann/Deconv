...
function [myReg,myRegGrad]=RegularizeGR(toRegularize,BetaVals)
        ...
        if (ndims(toRegularize) == 2) || (size(toRegularize,3) == 1)
            if (ndims(toRegularize) == 2)
                ...
                myRegGrad = ((1/(2*BetaVals(1)*BetaVals(1)))*(2*toRegularize-circshift(toRegularize,[-2 0]) - circshift(toRegularize,[2 0])) + ...
                    (1/(2*BetaVals(2)*BetaVals(2)))*(2*toRegularize-circshift(toRegularize,[0 -2]) - circshift(toRegularize,[0 2]))) ./ toRegularize - ...
                    ((aGrad{1}).^2+(aGrad{2}).^2)./(eps+toRegularize .* toRegularize);
                ...
                myRegGrad = ((1/(2*BetaVals(1)*BetaVals(1)))*(2*toRegularize-circshift(toRegularize,[-2 0 0]) - circshift(toRegularize,[2 0 0])) + ...
                    (1/(2*BetaVals(2)*BetaVals(2)))*(2*toRegularize-circshift(toRegularize,[0 -2 0]) - circshift(toRegularize,[0 2 0]))) ./ toRegularize - ...
                    ((aGrad{1}).^2+(aGrad{2}).^2)./(eps+toRegularize .* toRegularize);
            end
        
            % myReg = sum(BetaVals(1)*aGrad{1} .* aGrad{1} + BetaVals(2)*aGrad{2} .* aGrad{2});
            myReg = sum((abssqr(aGrad{1}) + abssqr(aGrad{2})) ./ (eps+abs(toRegularize)));
        elseif ndims(toRegularize) == 3
            ...
            myReg = sum((abssqr(aGrad{1}) + abssqr(aGrad{2}) + abssqr(aGrad{3})) ./ (eps+abs(toRegularize)));
            myRegGrad = ((1/(2*BetaVals(1)*BetaVals(1)))*(2*toRegularize-circshift(toRegularize,[-2 0 0])-circshift(toRegularize,[2 0 0])) + ...
                        (1/(2*BetaVals(2)*BetaVals(2)))*(2*toRegularize-circshift(toRegularize,[0 -2 0])-circshift(toRegularize,[0 2 0])) + ...
                        (1/(2*BetaVals(3)*BetaVals(3)))*(2*toRegularize-circshift(toRegularize,[0 0 -2])-circshift(toRegularize,[0 0 2]))) ./ toRegularize - ...
                        ((aGrad{1}).^2+(aGrad{2}).^2+(aGrad{3}).^2)./(eps+toRegularize .* toRegularize);
        else % 1-D
            aGrad{1}=(circshift(toRegularize,1)-circshift(toRegularize,-1))/(2*BetaVals(1));  % cyclic rotation
            myReg = sum(abssqr(aGrad{1}) ./ (eps+abs(toRegularize)));
            myRegGrad = ((1/(2*BetaVals(1)*BetaVals(1)))*(2*toRegularize-circshift(toRegularize,-2) - circshift(toRegularize,2))) ./ toRegularize - ...
                        ((aGrad{1}).^2)./(eps+toRegularize .* toRegularize);
        end 
